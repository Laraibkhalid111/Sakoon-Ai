...
def send_email_report(
    ...
    try:
        ...
        if not sender_email or not app_password:
            log.error("SMTP credentials missing from environment or st.secrets.")
            return False
        lang = session_data.get("session_language", "english")
        raw_name = session_data.get("name") or ("پیارے دوست" if lang == "urdu" else "there")
        name_plain = str(raw_name)
        name = html.escape(name_plain)
        narrative_plain = narrative or (
            "خلاصہ دستیاب نہیں ہے۔" if lang == "urdu" else "No narrative summary available."
        )
        narrative_display = html.escape(str(narrative_plain))
        ...
        if lang == "urdu":
            ...
            # narrative_display already set above (HTML-escaped)
            
            symptoms = session_data.get("symptoms", [])
            if symptoms:
                symptoms_html = "".join(
                    f"<li style='margin-bottom: 6px; direction: rtl; text-align: right;'>{html.escape(str(s))}</li>"
                    for s in symptoms
                )
            else:
            ...
            if triggers:
                triggers_html = "".join(
                    f"<li style='margin-bottom: 6px; direction: rtl; text-align: right;'>{html.escape(str(t))}</li>"
                    for t in triggers
                )
            else:
            ...
            care_html = "".join(f"<li style='margin-bottom: 6px; direction: rtl; text-align: right;'>{html.escape(item)}</li>" for item in care_items_ur)
            ...
            # Trusted static copy may include intentional <strong>; only escape plain items
            support_html = "".join(
                f"<li style='margin-bottom: 6px; direction: rtl; text-align: right;'>"
                f"{item if '<strong>' in item else html.escape(item)}</li>"
                for item in support_items_ur
            )
            ...
            if symptoms:
                symptoms_html = "".join(
                    f"<li style='margin-bottom: 6px;'>{html.escape(str(s))}</li>" for s in symptoms
                )
            else:
            ...
            if triggers:
                triggers_html = "".join(
                    f"<li style='margin-bottom: 6px;'>{html.escape(str(t))}</li>" for t in triggers
                )
            else:
                triggers_html = "<li style='margin-bottom: 6px;'>No specific triggers noted.</li>"
                
            care_html = "".join(f"<li style='margin-bottom: 6px;'>{html.escape(item)}</li>" for item in care_items)
            support_html = "".join(
                f"<li style='margin-bottom: 6px;'>{item if '<strong>' in item else html.escape(item)}</li>"
...
"""
        ...
        log.info("Email wellness report successfully sent to %s", recipient)
        return True
        
    except Exception as e:
        log.error("Failed to send email to %s: %s\n%s", recipient, e, traceback.format_exc())
        return False
